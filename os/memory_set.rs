...
use super::{VirtAddr, VirtPageNum, PhysAddr, PhysPageNum};
...
use riscv::register::sat;
use bitflags::bitflags;

// Permission flags for memory areas
bitflags! {
...
// A continuous virtual memory segment
pub struct MapArea {
    vpn_range: VPNRange,
...
impl MapArea {
    ...
    pub fn unmap(&mut self, page_table: &mut PageTable) {
        for vpn in self.vpn_range {
            self.unmap_one(page_table);
        }
    }

    // Copy data to mapped frames
...
// Address space (Page table + memory areas)
...
impl MemorySet {
    ...
    // Get SATP token for address space switch
    pub fn token(&self) -> usize {
        self.page_table.token()
    }

    // Insert framed memory area
    ...
    // Map trampoline page for trap switch
    ...
    // Activate current address space (write to satp)
    ...
    // Create kernel address space (identity mapping)
    ...
    // Create user address space from ELF file
...
// Global kernel address space (spin::Lazy instead of lazy_static)
pub static KERNEL_SPACE: Lazy<Arc<UPSafeCell<MemorySet>>> =
    Lazy::new(|| Arc::new(unsafe { UPSafeCell::new(MemorySet::new_kernel()) }));

// Remap test for page permission check
#[allow(unused)]
pub fn remap_test() {
    extern "C" {
        fn stext();
    ...
    let mid_text: VirtAddr = ((stext as *const () + etext as *const ()) / 2).into();
    let mid_rodata: VirtAddr = ((srodata as *const () + erodata as *const ()) / 2).into();
    let mid_data: VirtAddr = ((sdata as *const () + edata as *const ()) / 2).into();
